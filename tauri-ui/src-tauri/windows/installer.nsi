...
; FairyPam vendors Tauri CLI 2.11.4's template because product resources must never
; be unpacked through /D, a remembered registry value, or the directory page.
...
; Never uninstall an active product before the protected replacement stage has
; been fully verified and activated by NSIS_HOOK_ACTIVATE.
!if 0
...
; 5. Start menu shortcut page. The product install root is intentionally not editable.
...
Function un.onInit
  ; ponytail: uninstall only the fixed protected product root; `_?=` must not redirect deletion.
