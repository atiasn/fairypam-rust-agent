...
| 受管产品安装器与状态根 | pending | static security re-review, `WINDOWS-BUILD`, `WINDOWS-LIVE-SMOKE` | 设计的 CI candidate 为 `perMachine`，产品文件始终位于单一 `C:\Program Files\FairyPam` 根，状态位于 `C:\ProgramData\FairyPam.Agent\Agent`。WebView2 bootstrapper 固定为 `skip` 并由模板编译期拒绝非 `skip`；首次根创建时应用最终 DACL，重装时仅把三个固定目录的当前用户精确旧 ACL 收敛为私有值，拒绝 reparse、额外/未知写 ACE 和注册/审计目录异常，并在后续失败时回滚本次 ACL 修改；回滚失败使用独立 detail `10`，不伪装成原目录拒绝；产品布局仍无 slot Rename 或递归清理。 | 在标准 Windows 账户验证 fresh install、旧 ACL 同根升级、恶意 ACL 拒绝与完整 SDDL 回滚、旧 `.installing`/`.previous`/旧目录不阻塞安装且不被删除，以及产品根/状态根 ACL、DACL/reparse 和 UAC。 |
