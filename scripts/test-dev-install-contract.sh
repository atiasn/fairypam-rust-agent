...
if grep -Fq -- 'InheritanceFlags]::ContainerInherit' "$provision" || grep -Fq -- 'InheritanceFlags]::ObjectInherit' "$provision"; then
  printf '[ERROR] Dev private state directories must not expose inheritable ACEs\n' >&2
  exit 1
fi
