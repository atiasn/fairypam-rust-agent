...
fn profile_for_process(profiles: &ProfileStore, process_name: &str) -> Option<String> {
    let mut matches = profiles.ids().into_iter().filter(|profile_id| {
    ...
    let profile_id = matches.next()?;
    matches.next().is_none().then_some(profile_id)
}
...
mod tests {
    ...
    fn game_support_requires_a_verified_profile_process_mapping() {
        ...
        assert_eq!(
            profile_for_process(&profiles, "yuanshen.exe").as_deref(),
            Some("profile-a")
        );
