...
use fairypam_agent_protocol::{
    internal_v1 as internal,
    v3::{self as wire, command_identity, hub_control_command},
...
pub enum TranslatedCommand {
    ...
    RenewRealtimeProgram {
        task: internal::TaskCommandRef,
        value: wire::RenewRealtimeProgram,
    },
    StopRealtimeProgram {
        task: internal::TaskCommandRef,
        value: wire::StopRealtimeProgram,
    },
}
...
pub fn identity(command: &wire::HubControlCommand) -> Option<wire::CommandIdentity> {
    ...
    match command.payload.as_ref()? {
        ...
        Payload::StartRealtimeProgram(value) => value.reference.clone(),
        Payload::RenewRealtimeProgram(value) => value.reference.clone(),
...
pub fn command_name(command: &wire::HubControlCommand) -> &'static str {
    ...
    match command.payload.as_ref() {
        ...
        Some(Payload::StartRealtimeProgram(_)) => "start_realtime_program",
        Some(Payload::RenewRealtimeProgram(_)) => "renew_realtime_program",
    ...
    let payload = match command.payload.as_ref() {
        ...
        Some(Payload::StartCapture(value)) => {
                ...
                || !(1..=100).contains(&value.quality)
            {
                return Err(AgentError::new(
                    ...
                    "capture command is outside v3 bounds",
                ));
            }
        ...
        Some(Payload::CaptureFrame(value)) => {
                ...
                || !(1..=100).contains(&value.quality)
            {
                return Err(AgentError::new(
                    "capture.command_invalid",
                    "single-frame capture command is outside v3 bounds",
                ));
            }
        ...
        Some(Payload::StartRealtimeProgram(value)) => {
            translator.require_capability(value.reference.as_ref(), 8)?;
            if value.program_id != "genshin.music-autoplay.v1"
                || value.program_schema_version != 1
                || !is_digest(&value.program_digest)
                || !(1_000..=600_000).contains(&value.maximum_duration_ms)
                || value.target_generation == 0
            {
                return Err(AgentError::new(
                    "realtime.program_invalid",
                    "realtime program is not an installed bounded program",
                ));
            }
            let supervision_lease_ms = value.supervision_lease_ms.ok_or_else(|| {
                AgentError::new(
                    "realtime.program_invalid",
                    "realtime supervision lease must be present",
                )
            })?;
            if supervision_lease_ms != 0 && !(500..=5_000).contains(&supervision_lease_ms) {
                return Err(AgentError::new(
                    "realtime.program_invalid",
                    "realtime supervision lease must be 0 or between 500 and 5000 milliseconds",
                ));
            }
            return Ok(TranslatedCommand::StartRealtimeProgram {
        ...
        Some(Payload::RenewRealtimeProgram(value)) => {
            translator.require_capability(value.reference.as_ref(), 8)?;
            if value.program_id != "genshin.music-autoplay.v1"
                || !(500..=5_000).contains(&value.supervision_lease_ms)
            {
                return Err(AgentError::new(
                    "realtime.program_invalid",
                    "realtime renewal is outside the installed program policy",
                ));
            }
            return Ok(TranslatedCommand::RenewRealtimeProgram {
                task: internal_task(wire_task(value.reference.as_ref())?)?,
                value: value.clone(),
            });
        }
        Some(Payload::StopRealtimeProgram(value)) => {
            ...
            if value.program_id != "genshin.music-autoplay.v1" {
                return Err(AgentError::new(
                    "realtime.program_invalid",
                    "realtime stop does not name the installed program",
                ));
            }
            return Ok(TranslatedCommand::StopRealtimeProgram {
                task: internal_task(wire_task(value.reference.as_ref())?)?,
                value: value.clone(),
            });
        }
        ...
        Some(Payload::ClientPointClick(value)) => {
                ...
                || value.target_generation == 0
            {
                return Err(AgentError::new(
                    "input.frame_invalid",
                    "client point click is outside the v3 fixed bounds",
                ));
            }
...
impl Translator {
    ...
    fn translate_input_frame(
        ...
        if value.input_sequence == 0
            ...
            || !canonical_action_ids(&value.held_action_ids)
...
fn canonical_action_ids(action_ids: &[String]) -> bool {
    action_ids
        .iter()
        .all(|value| !value.is_empty() && value.len() <= 128)
        && action_ids.windows(2).all(|pair| pair[0] < pair[1])
}
...
fn reference_invalid() -> AgentError {
    AgentError::new(
        ...
        "v3 command identity does not match the command kind",
    )
}
...
mod tests {
    ...
    fn with_digest(mut command: wire::HubControlCommand) -> wire::HubControlCommand {
        let (identity, kind, payload) = match command.payload.as_ref().unwrap() {
            ...
            hub_control_command::Payload::RenewRealtimeProgram(value) => (
                value.reference.as_ref().unwrap(),
                "RenewRealtimeProgram",
                serde_json::json!({
                    "program_id": value.program_id,
            ...
            hub_control_command::Payload::StopRealtimeProgram(value) => (
                value.reference.as_ref().unwrap(),
                "StopRealtimeProgram",
    ...
    fn historical_digest_conflict_event_matches_the_shared_backend_contract() {
        let vector: serde_json::Value = serde_json::from_str(include_str!(
            "../../../proto/fairypam/agent/v3/testdata/historical-digest-conflict.json"
        ))
        .unwrap();
    ...
    #[test]
    fn input_frame_rejects_noncanonical_action_ids() {
        let frame = wire::InputFrame {
            held_action_ids: vec!["movement.forward".into(), "combat.attack".into()],
            ..wire::InputFrame::default()
        };

        assert!(!canonical_action_ids(&frame.held_action_ids));
    }

    #[test]
    fn translator_accepts_bounded_realtime_program_commands() {
        let contract = contract(vec![1, 8]);
        let mut translator = Translator::new(500);
        accept_begin(&mut translator, &contract);
        let start = translator
            .translate(&with_digest(wire::HubControlCommand {
                payload: Some(hub_control_command::Payload::StartRealtimeProgram(
                    wire::StartRealtimeProgram {
        ...
        assert!(matches!(
            start,
            TranslatedCommand::StartRealtimeProgram { .. }
        ));

        let renew = translator
            .translate(&with_digest(wire::HubControlCommand {
                payload: Some(hub_control_command::Payload::RenewRealtimeProgram(
                    wire::RenewRealtimeProgram {
        ...
        assert!(matches!(
            renew,
            TranslatedCommand::RenewRealtimeProgram { .. }
        ));

        let stop = translator
            .translate(&with_digest(wire::HubControlCommand {
                payload: Some(hub_control_command::Payload::StopRealtimeProgram(
                    wire::StopRealtimeProgram {
        ...
        assert!(matches!(
            stop,
            TranslatedCommand::StopRealtimeProgram { .. }
        ));
    }

    #[test]
    fn translator_rejects_unapproved_realtime_program() {
    ...
    #[test]
    fn translator_preserves_semantic_input_frame() {
        let contract = contract(vec![1, 4, 6]);
        let mut translator = Translator::new(500);
        accept_begin(&mut translator, &contract);
        let command = with_digest(wire::HubControlCommand {
            payload: Some(hub_control_command::Payload::InputFrame(wire::InputFrame {
                ...
                wheel_action_id: "inventory.scroll".into(),
                wheel_delta: -120,
        ...
        assert_eq!(frame.held_action_ids, ["movement.forward"]);
    ...
    #[test]
    fn translator_preserves_action_bound_client_point_click() {
        ...
        let command = with_digest(wire::HubControlCommand {
            payload: Some(hub_control_command::Payload::ClientPointClick(
                wire::ClientPointClick {
                    ...
                    input_sequence: 1,
                    lease_ms: 250,
                    action_id: "combat.normal_attack".into(),
                    x_ppm: 500_000,
                    y_ppm: 583_333,
    ...
    #[test]
    fn translator_rejects_capture_values_outside_v3_bounds() {
