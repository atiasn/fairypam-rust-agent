...
use crate::{ActionId, ActionMap, InputPlatform, ReleaseReason, ResolvedAction};
...
impl<P: InputPlatform, G: GuardianClient> LeaseExecutor<P, G> {
    ...
    fn apply_lease_inner(
        ...
        for action in &lease.desired_holds {
            self.actions.hold_scan_code(action)?;
        }
    ...
    fn apply_hold_difference(
        ...
        for action in previous.difference(desired) {
            self.platform
                .release_scan_code(self.actions.hold_scan_code(action)?)?;
        }
        for action in desired.difference(previous) {
            self.platform
                .press_scan_code(self.actions.hold_scan_code(action)?)?;
        }
        Ok(())
    }
    ...
    fn execute_pulse_inner(
        ...
        match self.actions.resolve(action)? {
            ResolvedAction::PulseScanCode(scan_code) => self.platform.pulse_scan_code(*scan_code),
