...
impl SendInputPlatform {
    ...
    fn keyboard(scan_code: u16, key_up: bool) -> INPUT {
...
impl InputPlatform for SendInputPlatform {
    fn press_scan_code(&mut self, scan_code: u16) -> Result<(), SafetyError> {
        self.send(&[Self::keyboard(scan_code, false)])
    }

    fn release_scan_code(&mut self, scan_code: u16) -> Result<(), SafetyError> {
        self.send(&[Self::keyboard(scan_code, true)])
    }

    fn pulse_scan_code(&mut self, scan_code: u16) -> Result<(), SafetyError> {
        self.send(&[
            Self::keyboard(scan_code, false),
            Self::keyboard(scan_code, true),
        ])
    }
    ...
    fn emergency_release(&mut self, scan_codes: &[u16]) -> Result<(), SafetyError> {
        let mut inputs = scan_codes
            .iter()
            .map(|scan_code| Self::keyboard(*scan_code, true))
            .collect::<Vec<_>>();
        inputs.extend([
            Self::mouse(MOUSEEVENTF_LEFTUP, 0, 0),
            Self::mouse(MOUSEEVENTF_RIGHTUP, 0, 0),
            Self::mouse(MOUSEEVENTF_MIDDLEUP, 0, 0),
        ]);
        self.send(&inputs)
    }

    fn relative_mouse(&mut self, delta_x: i32, delta_y: i32) -> Result<(), SafetyError> {
        self.send(&[Self::mouse(MOUSEEVENTF_MOVE, delta_x, delta_y)])
    }
    ...
    fn client_point_click(
        ...
        let (down, up) = match button {
            SemanticMouseButton::Left => (MOUSEEVENTF_LEFTDOWN, MOUSEEVENTF_LEFTUP),
            SemanticMouseButton::Right => (MOUSEEVENTF_RIGHTDOWN, MOUSEEVENTF_RIGHTUP),
            SemanticMouseButton::Middle => (MOUSEEVENTF_MIDDLEDOWN, MOUSEEVENTF_MIDDLEUP),
        };
        self.send(&[
            Self::mouse(
